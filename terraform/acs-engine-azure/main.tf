...
resource "azure_resource_group" "default" {
...
resource "null_resource" "render_acs_engine_config" {
    provisioner "local-exec" {
        command = "echo '${data.template_file.acs_engine_config.rendered}provider' > \
            ${var.acs_engine_config_file_rendered}"
    }
...
resource "null_resource" "deploy_acs" {
    provisioner "local-exec" {
        command = "az network vnet subnet update \
            ...
            --vnet-name ${azurerm_virtual_network.default.name} \
            --route-table $(az resource list \
                --resource-group ${var.resource_group_name} \
                --resource-type Microsoft.Network/routeTables | jq -r '.[] | .id')"
    }
